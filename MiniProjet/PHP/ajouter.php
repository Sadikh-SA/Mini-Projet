    ...
    <body>
        <div class="container">
            ...
            <div class="ajout" style="height: 450px;">
                ...
                <form class="form-horizontal" action="ajouter.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">          
                            <input type="number" class="form-control" id="prix" placeholder="Enter Prix du Produit" name="prix" min="50000">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-sm-2" for="pwd">Quantité du Produit</label>
                        <div class="col-sm-10">          
                            <input type="number" class="form-control" id="quantité" placeholder="Enter Prix du Produit" name="quantité" min="0" max="100">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-sm-2" for="pwd">Image du Produit</label>
                        <div class="col-sm-10">          
                            <input type="file" class="form-control" id="quantité" placeholder="Enter L'image du Produit" name="image">
                        </div>
                    </div>
                    <div class="form-group" >
                        <label class="control-label col-sm-2" for="pwd"></label>
                        <div class="col-sm-10" style="width:20%;">          
                            <button type="submit" class="btn btn-succes" name="submit" style="height:60px; background:#2E2E2E; color:white; font-size: 25px; margin-top: 15px;">Ajouter</button> 
                        </div>
                    </div>
        ...
        <?php
            ...
            if(isset($_POST['submit'])) {
                ...
                $uploaddir=$_FILES["image"]["tmp_name"];
                $image = basename($_FILES['image']['name']);
                $image=move_uploaded_file($tmp_name, "../Images/$image");
                for($i=0;$i<count($produit);$i++){
                    if(strtolower($nom) == strtolower($produit[$i][0])) {
                        $nom1=$nom1+1;
                    }
                }
                if($nom1!=0) {
                    echo "<h1 style='background: #2E2E2E; color: #FFF; text-align: center; text-transform:uppercase;'>Ce Produit $nom existe déja dans la base</h1><br>";
                }
                else {
                    $produit = array(   
                        array('Hp Probook G5',450000,9),
                        ...
                        array('Tablette',130000,80),
                        array('Ordinateur Bureau',200000,8),
                        array($nom,$prix,$quantité)
                    );
                    $images = array('../Images/hpg5.jpg','../Images/mac.jpg','../Images/dell.jpg','../Images/ip6.jpg','../Images/ip8.png','../Images/s8.jpg','../Images/j7.jpg','../Images/tab.jpg','../Images/buro.jpg','../Images/logo.png','"$image"');
                    echo "<h1 style='text-align:center; text-transform: uppercase; background: #2E2E2E; color:#FFF;'>Insertion Faite avec succès</h1>";
                    for($i=0;$i<count($produit);$i++){
                        if($i==0 || $i%3==0) {
